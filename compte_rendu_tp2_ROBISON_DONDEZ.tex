...
Le syndrome obtenu est 110 donc bit position 6
...
Puisque H est ordonnée on obtient directement la position de l'erreur en binaire
sur le vecteur colonne correspondant en binaire
\item Le mot corrigé est-il identique au mot codé initial ? 

Le mot corrigé est forcément identique au mot codé initial si uniquement une erreur
a été injectée.
\end{enumerate}
...
Hamming simple (sans 8ème bit) est incapable de corriger ou détecter 2 erreurs.
...
Le bit supplémentaire permet de détecter une deuxième erreur
...
\section{Partie 7 - Étude automatisée sur plusieurs messages}
\begin{enumerate}
...
Pour deux erreurs elles sont faussement corrigées
...
Le CRC est beaucoup plus fort comme solution pour détecter les erreurs. Elle marche aussi pour les multiples erreurs
...
Les polynômes peuvent être choisis suivant l'application donnée
Le CRC néanmoins ne permet pas de corriger

Hamming contrairement au CRC permet non seulement de détecter une erreur mais de la corriger
par contre il peut uniquement détecter et corriger une erreur simple

Hamming étendu peut détecter et corriger une erreur d'un bit mais il peut aussi détecter 2 erreurs
...
Hamming étendu permet de discriminer le cas de deux erreurs
\item Pourquoi le CRC reste-t-il utile même s’il ne corrige pas ? 

Le CRC reste utile puisqu'il est facile à implémenter en hardware et très flexible en terme de polynôme
...
On préférerait un code plus puissant dans le cas où l'on trouve souvent 2+ erreurs ou que la perte de données est inacceptable.
\end{enumerate}
...
La liaison satellitaire peut être sensible à énormément de bruit, le signal de celui-ci sera aussi bien faible
...
Si la modulation est adaptée, hamming pourrait peut-être suffire? Sinon on choisira un autre code plus puissant

La liaison CAN utilise un CRC en pratique.
Ce polynôme est conçu pour être robuste contre le bruit typique présent dans les voitures.
Le raisonnement derrière semble être une priorisation de sécurité avant la vitesse.
On préfère allouer tous les bits vers une détection plutôt qu'une correction
On pourrait pas du tout imaginer hamming par exemple dans utiliser un code hamming.
Celui-ci n'est pas du tout suffisamment robuste
...
\item Quel type d’erreurs est probable dans chaque cas ? 
	...
	La liaison satellitaire est sensible à du \textit{bit flipping} (erreur simple bit).
	CAN et le stockage SSD sont sensibles à des erreurs de bits multiples.
	Le CAN puisqu'il y a du bruit qui pourrait porter sur une durée supérieure au débit.
	C'est un bruit particulier dont les sources sont connues (bougie voiture, moteurs fenêtre).
	Les SSD ont des erreurs de plusieurs bits sur un secteur après usure. L'information à moins tendance à rester.
\item Quelle contrainte système est importante (latence, robustesse, complexité, débit, énergie) ?

	La SSD est fortement impacté par la latence. Le CAN a besoin de robustesse, on néglige à la fois la latence et l'impact de la retransmission sur le débit. Sur les satellites ça dépend mais la robustesse et la latence est importante. L'énergie l'est aussi donc on va avoir tendance à minimiser le nombre de transmission depuis les satellites et donc assurer la redondance sur le premier coup.
...
Hamming pourrait peut-être suffire en satellitaire (à voire).
\end{enumerate}
\end{document}
